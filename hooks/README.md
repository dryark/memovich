# Memovich Hooks — Auto-Save for Terminal AI Tools

These hook scripts make Memovich save automatically. No manual "save" commands needed.
...
```json
{
  "hooks": {
    "Stop": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "/absolute/path/to/hooks/memovich_save_hook.sh",
        "timeout": 30
      }]
    }],
    "PreCompact": [{
      "hooks": [{
        "type": "command",
        "command": "/absolute/path/to/hooks/memovich_precompact_hook.sh",
        "timeout": 30
      }]
    }]
  }
...
```bash
chmod +x hooks/memovich_save_hook.sh hooks/memovich_precompact_hook.sh
```
...
Edit `memovich_save_hook.sh` to change:

- **`SAVE_INTERVAL=15`** — How many human messages between saves. Lower = more frequent saves, higher = less interruption.
- **`STATE_DIR`** — Where hook state is stored (defaults to `~/.memovich/hook_state/`)
- **`MEMOVICH_DIR`** — Optional. Set to a conversations directory to auto-run `memovich mine <dir>` on each save trigger. Leave blank (default) to let the AI handle saving via the block reason message.
...
```bash
memovich mine <dir>               # Mine all files in a directory
memovich mine <dir> --mode convos # Mine conversation transcripts only
```
...
Check the hook log:
```bash
cat ~/.memovich/hook_state/hook.log
```

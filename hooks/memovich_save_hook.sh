#!/bin/bash
# MEMOVICH SAVE HOOK — Auto-save every N exchanges
#
...
#       "hooks": [{
#         "type": "command",
#         "command": "/absolute/path/to/memovich_save_hook.sh",
#         "timeout": 30
#       }]
#     }]
#   }
...
#
# === MEMOVICH CLI ===
# This repo uses: memovich mine <dir>
# or:            memovich mine <dir> --mode convos
# Set MEMOVICH_DIR below if you want the hook to auto-ingest after blocking.
# Leave blank to rely on the AI's own save instructions.
#
# === CONFIGURATION ===

SAVE_INTERVAL=15  # Save every N human messages (adjust to taste)
STATE_DIR="$HOME/.memovich/hook_state"
mkdir -p "$STATE_DIR"

# Optional: set to the directory you want auto-ingested on each save trigger.
# Example: MEMOVICH_DIR="$HOME/conversations"
...
# Log for debugging (check ~/.memovich/hook_state/hook.log)
...
if [ "$SINCE_LAST" -ge "$SAVE_INTERVAL" ] && [ "$EXCHANGE_COUNT" -gt 0 ]; then
    ...
    # Optional: run memovich ingest in background if MEMOVICH_DIR is set
    if [ -n "$MEMOVICH_DIR" ] && [ -d "$MEMOVICH_DIR" ]; then
        SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
        REPO_DIR="$(dirname "$SCRIPT_DIR")"
        python3 -m memovich mine "$MEMOVICH_DIR" >> "$STATE_DIR/hook.log" 2>&1 &
    fi

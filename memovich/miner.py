...
Reads memovich.yaml from the project directory to know the wing + rooms.
...
def load_config(project_dir: str) -> dict:
    """Load memovich.yaml from project directory."""
    import yaml

    config_path = Path(project_dir).expanduser().resolve() / "memovich.yaml"
    if not config_path.exists():
        print(f"ERROR: No memovich.yaml found in {project_dir}")
        print(f"Run: memovich init {project_dir}")
        sys.exit(1)
...
def mine(
    ...
    print('\n  Next: memovich search "what you\'re looking for"')
    print(f"{'=' * 55}\n")
...
def status(palace_path: str):
    ...
    print(f"\n{'=' * 55}")
    print(f"  Memovich Status — {len(metas)} chunks")
    print(f"{'=' * 55}\n")

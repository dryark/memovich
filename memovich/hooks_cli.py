"""
Hook logic for Memovich — Python implementation of session-start, stop, and precompact hooks.
...
def _maybe_auto_ingest():
    """If MEMOVICH_DIR is set and exists, run memovich mine in background."""
    mine_dir = os.environ.get("MEMOVICH_DIR", "")
    if mine_dir and os.path.isdir(mine_dir):
        try:
            log_path = STATE_DIR / "hook.log"
            with open(log_path, "a") as log_f:
                subprocess.Popen(
                    [sys.executable, "-m", "memovich", "mine", mine_dir],
                    stdout=log_f,
                    stderr=log_f,
                )
        except OSError:
...
def hook_stop(data: dict, harness: str):
    ...
    if since_last >= SAVE_INTERVAL and exchange_count > 0:
        ...
        # Optional: auto-ingest if MEMOVICH_DIR is set
        _maybe_auto_ingest()

#!/usr/bin/env python3
"""
onboarding.py — Memovich first-run setup.

Asks the user:
  1. How they're using Memovich (work / personal / combo)
...
Seeds the entity_registry with confirmed data so Memovich knows your world
from minute one — before a single session is indexed.

Usage:
    python3 -m memovich.onboarding
    or: memovich init
"""

from pathlib import Path
from memovich.entity_registry import EntityRegistry
from memovich.entity_detector import detect_entities, scan_for_detection
...
def _ask_mode() -> str:
    _header("Welcome to Memovich")
    print("""
  Memovich is a personal memory system. To work well, it needs to know
...
""")
    print("  How are you using Memovich?")
    print()
...
def _ask_projects(mode: str) -> list:
    ...
    _hr()
    print("""
  What are your main projects? (These help Memovich distinguish project
...
def _warn_ambiguous(people: list) -> list:
    ...
    """
    from memovich.entity_registry import COMMON_ENGLISH_WORDS
...
def _generate_aaak_bootstrap(
    ...
    """
    memovich_dir = Path(config_dir) if config_dir else Path.home() / ".memovich"
    memovich_dir.mkdir(parents=True, exist_ok=True)
    ...
    registry_lines = [
        "# AAAK Entity Registry",
        "# Auto-generated by memovich init. Update as needed.",
        "",
        "## People",
    ]
    for p in people:
    ...
    (memovich_dir / "aaak_entities.md").write_text("\n".join(registry_lines), encoding="utf-8")
    ...
    (memovich_dir / "critical_facts.md").write_text("\n".join(facts_lines), encoding="utf-8")
...
def run_onboarding(
  ...
  Memovich will check the context before treating them as person names.
...
""")
    ...
    print("\n  AAAK entity registry: ~/.memovich/aaak_entities.md")
    print("  Critical facts bootstrap: ~/.memovich/critical_facts.md")

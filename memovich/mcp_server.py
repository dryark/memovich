#!/usr/bin/env python3
"""
Memovich MCP Server — read/write palace access for Claude Code
================================================================
Install: claude mcp add memovich -- python -m memovich.mcp_server [--palace /path/to/palace]

Tools (read):
  memovich_status          — total drawers, wing/room breakdown
  ...
  memovich_search          — semantic search, optional wing/room filter
  memovich_check_duplicate — check if content already exists before filing

Tools (write):
  memovich_add_drawer      — file verbatim content into a wing/room
  memovich_delete_drawer   — remove a drawer by ID

Tools (maintenance):
  memovich_reconnect       — force cache invalidation and reconnect after external writes
"""
...
from .config import MemovichConfig, sanitize_name, sanitize_content
...
logger = logging.getLogger("memovich_mcp")


def _parse_args():
    parser = argparse.ArgumentParser(description="Memovich MCP Server")
    parser.add_argument(
        "--palace",
        metavar="PATH",
...
if _args.palace:
    os.environ["MEMOVICH_PALACE_PATH"] = os.path.abspath(_args.palace)

_config = MemovichConfig()
# Only override KG path when --palace is explicitly provided; otherwise use
# KnowledgeGraph's default (~/.memovich/knowledge_graph.sqlite3).
if _args.palace:
...
_WAL_DIR = Path(os.path.expanduser("~/.memovich/wal"))
...
def _no_palace():
    return {
        ...
        "hint": "Run: memovich init <dir> && memovich mine <dir>",
    }
...
# Also available via memovich_get_aaak_spec tool.

PALACE_PROTOCOL = """IMPORTANT — Memovich Memory Protocol:
1. ON WAKE-UP: Call memovich_status to load palace overview + AAAK spec.
2. BEFORE RESPONDING about any person, project, or past event: call memovich_kg_query or memovich_search FIRST. Never guess — verify.
...
5. WHEN FACTS CHANGE: call memovich_kg_invalidate on the old fact, memovich_kg_add for the new one.

This protocol ensures the AI KNOWS before it speaks. Storage is not memory — but storage + this protocol = memory."""

AAAK_SPEC = """AAAK is a compressed memory dialect that Memovich uses for efficient storage.
...
def tool_memories_filed_away():
    ...
    state_dir = Path.home() / ".memovich" / "hook_state"
...
TOOLS = {
    ...
    "memovich_get_taxonomy": {
        "description": "Full taxonomy: namespace → segment → chunk count",
        "input_schema": {"type": "object", "properties": {}},
        "handler": tool_get_taxonomy,
    },
    "memovich_get_aaak_spec": {
        "description": "Get the AAAK dialect specification — the compressed memory format Memovich uses. Call this if you need to read or write AAAK-compressed memories.",
        "input_schema": {"type": "object", "properties": {}},
    ...
    if method == "initialize":
        ...
        return {
            ...
            "result": {
                ...
                "capabilities": {"tools": {}},
                "serverInfo": {"name": "memovich", "version": __version__},
            },
        }
    elif method == "ping":
...
def main():
    logger.info("Memovich MCP Server starting...")
    while True:
        try:

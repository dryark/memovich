...
Used by memovich init before mining begins.
...
PROJECT_VERB_PATTERNS = [
    ...
    r"\b{name}\.py\b",  # memovich.py
    r"\b{name}-core\b",  # memovich-core (hyphen only, not underscore)
    r"\b{name}-local\b",
    r"\bimport\s+{name}\b",
...
def confirm_entities(detected: dict, yes: bool = False) -> dict:
    ...
    """
    print(f"\n{'=' * 58}")
    print("  Memovich — Entity Detection")
    print(f"{'=' * 58}")

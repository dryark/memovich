...
Guide the user through a complete Memovich setup. Follow each step in order,
...
## Step 2: Check if memovich is already installed

Run `pip show memovich` to see if the package is already present. If it is,
report the installed version and skip to Step 4.

## Step 3: Install memovich

Run `pip install memovich`.

### Error handling -- pip failures

If `pip install memovich` fails, try these fallbacks in order:

1. Try `pip3 install memovich`
2. Try `python -m pip install memovich` (or `python3 -m pip install memovich`)
...
Ask the user which project directory they want to initialize with Memovich.
Offer the current working directory as the default. Wait for their response
before continuing.

## Step 5: Initialize the palace

Run `memovich init --yes <dir>` where `<dir>` is the directory from Step 4.

If this fails, report the error and stop.

## Step 6: Configure MCP server

Run the following command to register the Memovich MCP server with Claude:
...
Run `memovich status` and confirm the output shows a healthy palace.
...
- Use /memovich:mine to start adding data to their palace
- Use /memovich:search to query their palace and retrieve stored knowledge

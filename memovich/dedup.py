...
Usage (standalone):
    python -m memovich.dedup                          # dedup all
    python -m memovich.dedup --dry-run                # preview only
    python -m memovich.dedup --threshold 0.10         # stricter (near-identical only)
    python -m memovich.dedup --threshold 0.35         # looser (catches paraphrased content)
    python -m memovich.dedup --namespace my_project   # scope to one namespace
    python -m memovich.dedup --stats                  # stats only
    python -m memovich.dedup --source "my_project"    # filter by source

Usage (from CLI):
    memovich dedup [--dry-run] [--threshold 0.15] [--stats]
"""
...
from .metadata_keys import NAMESPACE
...
def _get_palace_path():
    ...
    try:
        ...
        return MemovichConfig().palace_path
    except Exception:
        return os.path.join(os.path.expanduser("~"), ".memovich", "palace")


def get_source_groups(col, min_count=MIN_DRAWERS_TO_CHECK, source_pattern=None, namespace=None):
    """Group chunks by source_file, return groups with min_count+ entries.

    If namespace is specified, only considers chunks in that namespace.
    """
    total = col.count()
    ...
    while offset < total:
        ...
        if namespace:
            kwargs["where"] = {NAMESPACE: namespace}
...
def show_stats(palace_path=None):
    ...
    cfg = MemovichConfig()
    col = get_collection(palace_path, collection_name=cfg.collection_name, create=False, config=cfg)
...
def dedup_palace(
    ...
    """Main entry point: deduplicate near-identical chunks across the store."""
    palace_path = palace_path or _get_palace_path()
    cfg = MemovichConfig()

    print(f"\n{'=' * 55}")
    print("  Memovich Deduplicator")
    print(f"{'=' * 55}")
    ...
    groups = get_source_groups(col, min_count, source_pattern, namespace=namespace)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--namespace", default=None, help="Scope dedup to a single namespace")

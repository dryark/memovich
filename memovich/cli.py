#!/usr/bin/env python3
"""
Memovich — Give your AI a memory. No API key required.

Two ways to ingest:
  Projects:      memovich mine ~/projects/my_app          (code, docs, notes)
  Conversations: memovich mine ~/chats/ --mode convos     (Claude, ChatGPT, Slack)

Same palace. Same search. Different ingest strategies.

Commands:
    memovich init <dir>                  Detect rooms from folder structure
    memovich split <dir>                 Split concatenated mega-files into per-session files
    memovich mine <dir>                  Mine project files (default)
    memovich mine <dir> --mode convos    Mine conversation exports
    memovich search "query"              Find anything, exact words
    memovich mcp                         Show MCP setup command
    memovich wake-up                     Show L0 + L1 wake-up context
    memovich wake-up --wing my_app       Wake-up for a specific project
    memovich status                      Show what's been filed

Examples:
    memovich init ~/projects/my_app
    memovich mine ~/projects/my_app
    memovich mine ~/chats/claude-sessions --mode convos
    memovich search "why did we switch to GraphQL"
    memovich search "pricing discussion" --wing my_app --room costs
"""
...
def cmd_mine(args):
    palace_path = os.path.expanduser(args.palace) if args.palace else MemovichConfig().palace_path
    include_ignored = []
...
def cmd_repair(args):
    ...
    try:
        ...
        col = client.get_collection("memovich_chunks")
        total = col.count()
    ...
    client.delete_collection("memovich_chunks")
    new_col = client.create_collection("memovich_chunks")
...
def cmd_mcp(args):
    """Show how to wire Memovich into MCP-capable hosts."""
    base_server_cmd = "python -m memovich.mcp_server"
    ...
    print("Memovich MCP quick setup:")
    print(f"  claude mcp add memovich -- {server_cmd}")
    print("\nRun the server directly:")
    print(f"  {server_cmd}")

    if not args.palace:
        print("\nOptional custom palace:")
        print(f"  claude mcp add memovich -- {base_server_cmd} --palace /path/to/palace")
...
def cmd_compress(args):
    ...
    try:
        ...
        print("  Run: memovich init <dir> then memovich mine <dir>")
        sys.exit(1)
    ...
    if not args.dry_run:
        try:
            comp_col = get_collection(
                palace_path,
                collection_name="memovich_compressed",
                create=True,
                config=cfg,
            )
            ...
            print(
                f"  Stored {len(compressed_entries)} compressed chunks in collection 'memovich_compressed'."
            )
        except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(
        description="Memovich — Give your AI a memory. No API key required.",
    ...
    parser.add_argument(
        "--palace",
        default=None,
        help="Where the palace lives (default: from ~/.memovich/config.json or ~/.memovich/palace)",
    )
    ...
    p_mine.add_argument(
        "--namespace",
        default=None,
        help="Namespace (default: from memovich.yaml or directory name)",
    )
    p_mine.add_argument(
        "--no-gitignore",
        action="store_true",
    ...
    p_mine.add_argument(
        "--agent",
        default="memovich",
        help="Your name — recorded on every drawer (default: memovich)",
    )
    ...
    sub.add_parser(
        "mcp",
        help="Show MCP setup command for connecting Memovich to your AI client",
    )

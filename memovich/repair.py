...
Usage (standalone):
    python -m memovich.repair scan [--namespace X]
    python -m memovich.repair prune --confirm
    python -m memovich.repair rebuild

Usage (from CLI):
    memovich repair
    memovich repair-scan [--namespace X]
    memovich repair-prune --confirm
"""
...
def _get_palace_path():
    ...
    try:
        ...
        default = os.path.join(os.path.expanduser("~"), ".memovich", "palace")
        return default
...
def scan_palace(palace_path=None, only_namespace=None):
    ...
    if cfg.vector_backend != "chroma":
        print(f"\n  repair scan requires vector_backend=chroma (yours: {cfg.vector_backend!r}).")
        return set(), set()
    ...
    where = {NAMESPACE: only_namespace} if only_namespace else None
    total = col.count()
    print(f"  Collection: {COLLECTION_NAME}, total: {total:,}")
    if only_namespace:
        print(f"  Scanning namespace: {only_namespace}")
...
def rebuild_index(palace_path=None):
    ...
    print(f"\n{'=' * 55}")
    print("  Memovich Repair — Index Rebuild")
    print(f"{'=' * 55}\n")
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="Memovich repair tools")
    p.add_argument("command", choices=["scan", "prune", "rebuild"])
    p.add_argument("--palace", default=None, help="Palace directory path")
    p.add_argument("--namespace", default=None, help="Scan only this namespace")
    ...
    if args.command == "scan":
        scan_palace(palace_path=path, only_namespace=args.namespace)

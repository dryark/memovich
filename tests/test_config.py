...
import pytest
from memovich.config import MemovichConfig, sanitize_name


def test_default_config():
    cfg = MemovichConfig(config_dir=tempfile.mkdtemp())
    assert "palace" in cfg.palace_path
    assert cfg.collection_name == "memovich_chunks"
...
def test_config_from_file():
    ...
    cfg = MemovichConfig(config_dir=tmpdir)
    assert cfg.palace_path == "/custom/palace"


def test_env_override():
    os.environ["MEMOVICH_PALACE_PATH"] = "/env/palace"
    cfg = MemovichConfig(config_dir=tempfile.mkdtemp())

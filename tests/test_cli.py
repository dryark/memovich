"""Tests for memovich.cli — the main CLI dispatcher."""
...
@patch("memovich.cli.MemovichConfig")
...
def test_cmd_search_calls_search(mock_config_cls):
    ...
    args = argparse.Namespace(
        palace=None, query="test query", namespace="mywing", segment="myroom", results=3
    )
    with patch("memovich.searcher.search") as mock_search:
        cmd_search(args)
...
def test_cmd_search_error_exits(mock_config_cls):
    mock_config_cls.return_value.palace_path = "/fake/palace"
    args = argparse.Namespace(palace=None, query="q", namespace=None, segment=None, results=5)
    from memovich.searcher import SearchError

    with patch("memovich.searcher.search", side_effect=SearchError("fail")):
...
def test_cmd_instructions_calls_run_instructions():
    ...
    with patch("memovich.instructions_cli.run_instructions") as mock_run:
...
def test_cmd_hook_calls_run_hook():
    ...
    with patch("memovich.hooks_cli.run_hook") as mock_run:
        cmd_hook(args)
...
def test_cmd_init_no_entities(mock_config_cls, tmp_path):
    ...
    with (
        patch("memovich.entity_detector.scan_for_detection", return_value=[]),
        patch("memovich.room_detector_local.detect_rooms_local") as mock_rooms,
    ):
        cmd_init(args)
...
def test_cmd_init_with_entities(mock_config_cls, tmp_path):
    ...
    args = argparse.Namespace(dir=str(tmp_path), yes=True)
    with (
        patch("memovich.entity_detector.scan_for_detection", return_value=fake_files),
        patch("memovich.entity_detector.detect_entities", return_value=detected),
        patch("memovich.entity_detector.confirm_entities", return_value=confirmed),
        patch("memovich.room_detector_local.detect_rooms_local"),
...
def test_cmd_mine_projects_mode(mock_config_cls):
    ...
    with patch("memovich.miner.mine") as mock_mine:
        cmd_mine(args)
...
def test_cmd_mine_convos_mode(mock_config_cls):
    ...
    with patch("memovich.convo_miner.mine_convos") as mock_mine:
        cmd_mine(args)
        mock_mine.assert_called_once_with(
            convo_dir="/chats",
            palace_path="/fake/palace",
...
def test_cmd_wakeup(mock_config_cls, capsys):
    mock_config_cls.return_value.palace_path = "/fake/palace"
    args = argparse.Namespace(palace=None, namespace=None)
    mock_stack = MagicMock()
    mock_stack.wake_up.return_value = "Hello world context"
    with patch("memovich.layers.MemoryStack", return_value=mock_stack):
        cmd_wakeup(args)
    mock_stack.wake_up.assert_called_once_with(namespace=None)
    out = capsys.readouterr().out
...
def test_cmd_split_basic():
    ...
    with patch("memovich.split_mega_files.main") as mock_main:
        cmd_split(args)
...
def test_cmd_split_all_options():
    ...
    assert sys.argv[0] != "memovich split"
...
def test_main_no_args_prints_help(capsys):
    with patch("sys.argv", ["memovich"]):
        main()
    out = capsys.readouterr().out
    assert "Memovich" in out


def test_main_status_dispatches():
    with (
        patch("sys.argv", ["memovich", "status"]),
        patch("memovich.cli.cmd_status") as mock_cmd,
    ):
        main()
...
def test_main_search_dispatches():
    with (
        patch("sys.argv", ["memovich", "search", "my query"]),
        patch("memovich.cli.cmd_search") as mock_cmd,
    ):
        main()
        mock_cmd.assert_called_once()
...
def test_main_init_dispatches():
    with (
        patch("sys.argv", ["memovich", "init", "/some/dir"]),
        patch("memovich.cli.cmd_init") as mock_cmd,
    ):
        main()
        mock_cmd.assert_called_once()
...
def test_main_mine_dispatches():
    with (
        patch("sys.argv", ["memovich", "mine", "/some/dir"]),
        patch("memovich.cli.cmd_mine") as mock_cmd,
    ):
        main()
        mock_cmd.assert_called_once()
...
def test_main_wakeup_dispatches():
    with (
        patch("sys.argv", ["memovich", "wake-up"]),
        patch("memovich.cli.cmd_wakeup") as mock_cmd,
    ):
        main()
        mock_cmd.assert_called_once()
...
def test_main_split_dispatches():
    with (
        patch("sys.argv", ["memovich", "split", "/chats"]),
        patch("memovich.cli.cmd_split") as mock_cmd,
    ):
        main()
        mock_cmd.assert_called_once()
...
def test_mcp_command_prints_setup_guidance(monkeypatch, capsys):
    monkeypatch.setattr(sys, "argv", ["memovich", "mcp"])

    main()

    captured = capsys.readouterr()
    assert "Memovich MCP quick setup:" in captured.out
    assert "claude mcp add memovich -- python -m memovich.mcp_server" in captured.out
    assert "\nOptional custom palace:\n" in captured.out
    assert "python -m memovich.mcp_server --palace /path/to/palace" in captured.out
...
def test_mcp_command_uses_custom_palace_path_when_provided(monkeypatch, capsys):
    monkeypatch.setattr(sys, "argv", ["memovich", "--palace", "~/tmp/my palace", "mcp"])
    ...
    assert "python -m memovich.mcp_server --palace" in captured.out
...
def test_main_hook_no_subcommand_prints_help(capsys):
    with patch("sys.argv", ["memovich", "hook"]):
        main()
    out = capsys.readouterr().out
...
def test_main_hook_run_dispatches():
    with (
        patch(
            "sys.argv",
            ["memovich", "hook", "run", "--hook", "session-start", "--harness", "claude-code"],
        ),
        patch("memovich.cli.cmd_hook") as mock_cmd,
    ):
        main()
        mock_cmd.assert_called_once()
...
def test_main_instructions_no_subcommand_prints_help(capsys):
    with patch("sys.argv", ["memovich", "instructions"]):
        main()
    out = capsys.readouterr().out
...
def test_main_instructions_dispatches():
    with (
        patch("sys.argv", ["memovich", "instructions", "help"]),
        patch("memovich.cli.cmd_instructions") as mock_cmd,
    ):
        main()
        mock_cmd.assert_called_once()
...
def test_main_repair_dispatches():
    with (
        patch("sys.argv", ["memovich", "repair"]),
        patch("memovich.cli.cmd_repair") as mock_cmd,
    ):
        main()
        mock_cmd.assert_called_once()
...
def test_main_compress_dispatches():
    with (
        patch("sys.argv", ["memovich", "compress"]),
        patch("memovich.cli.cmd_compress") as mock_cmd,
    ):
        main()
        mock_cmd.assert_called_once()
...
def test_cmd_repair_success(mock_config_cls, tmp_path, capsys):
    ...
    mock_col.get.return_value = {
        "ids": ["id1", "id2"],
        "documents": ["doc1", "doc2"],
        "metadatas": [{"namespace": "a"}, {"namespace": "b"}],
    }
    mock_client = MagicMock()
    mock_client.get_collection.return_value = mock_col
...
@patch("memovich.palace.get_collection")
@patch("memovich.cli.MemovichConfig")
def test_cmd_compress_no_palace(mock_config_cls, mock_get_collection, capsys):
    mock_config_cls.return_value.palace_path = "/fake/palace"
    mock_config_cls.return_value.collection_name = "memovich_chunks"
    args = argparse.Namespace(palace=None, namespace=None, dry_run=False, config=None)
    mock_get_collection.side_effect = Exception("no palace")
...
@patch("memovich.palace.get_collection")
@patch("memovich.cli.MemovichConfig")
def test_cmd_compress_no_drawers(mock_config_cls, mock_get_collection, capsys):
    mock_config_cls.return_value.palace_path = "/fake/palace"
    mock_config_cls.return_value.collection_name = "memovich_chunks"
    args = argparse.Namespace(palace=None, namespace="mywing", dry_run=False, config=None)
    mock_col = MagicMock()
...
@patch("memovich.palace.get_collection")
@patch("memovich.cli.MemovichConfig")
def test_cmd_compress_dry_run(mock_config_cls, mock_get_collection, capsys):
    mock_config_cls.return_value.palace_path = "/fake/palace"
    mock_config_cls.return_value.collection_name = "memovich_chunks"
    args = argparse.Namespace(palace=None, namespace=None, dry_run=True, config=None)
    mock_col = MagicMock()
    mock_col.get.side_effect = [
        {
            "documents": ["some long text here for testing"],
            "metadatas": [{"namespace": "test", "segment": "general", "source_file": "test.txt"}],
            "ids": ["id1"],
        },
    ...
    with patch.dict("sys.modules", {"memovich.dialect": mock_dialect_mod}):
        cmd_compress(args)
    out = capsys.readouterr().out
...
@patch("memovich.palace.get_collection")
@patch("memovich.cli.MemovichConfig")
def test_cmd_compress_with_config(mock_config_cls, mock_get_collection, tmp_path, capsys):
    mock_config_cls.return_value.palace_path = "/fake/palace"
    mock_config_cls.return_value.collection_name = "memovich_chunks"
    config_file = tmp_path / "entities.json"
    config_file.write_text('{"people": [], "projects": []}')
    args = argparse.Namespace(palace=None, namespace=None, dry_run=True, config=str(config_file))
    mock_col = MagicMock()
    mock_col.get.return_value = {"documents": [], "metadatas": [], "ids": []}
    mock_get_collection.return_value = mock_col
...
@patch("memovich.palace.get_collection")
@patch("memovich.cli.MemovichConfig")
def test_cmd_compress_stores_results(mock_config_cls, mock_get_collection, capsys):
    """Non-dry-run compress stores to memovich_compressed collection."""
    mock_config_cls.return_value.palace_path = "/fake/palace"
    mock_config_cls.return_value.collection_name = "memovich_chunks"
    args = argparse.Namespace(palace=None, namespace=None, dry_run=False, config=None)
    mock_col = MagicMock()
    mock_col.get.side_effect = [
        {
            "documents": ["text"],
            "metadatas": [{"namespace": "w", "segment": "r", "source_file": "f.txt"}],
            "ids": ["id1"],
        },
        {"documents": [], "metadatas": [], "ids": []},
    ]
    ...
    def _gc(path, collection_name=None, create=False, config=None):
        if collection_name == "memovich_compressed":
            return mock_comp_col
        return mock_col

    mock_get_collection.side_effect = _gc
...
def test_cmd_repair_trailing_slash_does_not_recurse():
    ...
    with patch("memovich.cli.os.path.isdir", return_value=False):
        cmd_repair(args)

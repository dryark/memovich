...
106 tests that benchmark memovich at scale to validate real-world performance limits.

## Why

Memovich has strong academic scores (96.6% R@5 on LongMemEval) but no empirical data on how it behaves at scale. Key unknowns:
...
- **`generate_project_tree()`** — writes real files + `memovich.yaml` for `mine()` to ingest

"""Extra tests for memovich.config to cover remaining gaps."""

import json
import os

from memovich.config import MemovichConfig


def test_config_bad_json(tmp_path):
    """Bad JSON in config file falls back to empty."""
    (tmp_path / "config.json").write_text("not json", encoding="utf-8")
    cfg = MemovichConfig(config_dir=str(tmp_path))
...
def test_env_memovich_palace_path(tmp_path):
    """MEMOVICH_PALACE_PATH overrides config."""
    os.environ.pop("MEMOVICH_PALACE_PATH", None)
    os.environ["MEMOVICH_PALACE_PATH"] = "/env/palace/path"
    try:
        cfg = MemovichConfig(config_dir=str(tmp_path))
        assert cfg.palace_path == "/env/palace/path"
    finally:
        del os.environ["MEMOVICH_PALACE_PATH"]

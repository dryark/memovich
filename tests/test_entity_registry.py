"""Tests for memovich.entity_registry."""

from unittest.mock import patch

from memovich.entity_registry import (
...
def test_save_and_load_roundtrip(tmp_path):
    ...
    assert "Memovich" in loaded.projects
...
def test_lookup_known_project(tmp_path):
    registry = EntityRegistry.load(config_dir=tmp_path)
    registry.seed(mode="work", people=[], projects=["Memovich"])
    result = registry.lookup("Memovich")
...
def test_research_caches_result(tmp_path):
    ...
    with patch("memovich.entity_registry._wikipedia_lookup", return_value=mock_result):
    ...
    with patch(
        "memovich.entity_registry._wikipedia_lookup",
